...
app.post("/api/books", authenticateJWT, authorizeRoles("librarian"), async (req, res) => {
    ...
    if (!validator.isInt(String(isbn)))
      return res
        .status(400)
        .json({ message: "Index number must only contain numbers" });

    if (!validator.isInt(String(publishYear)))
      return res
        .status(400)
        .json({ message: "Year of publishing must be in numbers" });

    if (publishYear > currentYear)
      return res
        .status(400)
        .json({ message: "Book cannot be published in future" });
    ...
    if (existing)
      return res
        .status(400)
        .json({ message: "Book by this isbn already exists" });
...
app.get("/api/loans/my", authenticateJWT, async (req, res) => {
  try {
    ...
    const result = loans.map((loan) => {
      const isOverdue = loan.status === "active" && loan.due_date < now;
...
app.put("/api/loans/:id/return", authenticateJWT, async (req, res) => {
  ...
  try {
    ...
    if (loan.user_id !== userId) {
      ...
      return res
        .status(403)
        .json({ message: "Cannot return someone else's loan" });
    }
